...
fn main() -> () {
    ...
    let action_list = data::action::Action::init_action_list(&chummer_path).expect("Could not import actions");
    println!("Imported {} actions.", action_list.len());

    println!("Importing Programs.");
    let program_list = data::program::Program::init_program_list(&chummer_path).expect("Could not import programs");
